...
class CouponsInputForm(forms.Form):
    ...
    service_ratio = forms.IntegerField(min_value=1, max_value=100,
                                       error_messages={'required': u'平台商承担优惠比例值不能为空',
                                                       'max_value': u'service_ratio取值范围为1~100'})
    business_ratio = forms.IntegerField(min_value=1, max_value=100,
                                        error_messages={'required': u'商户承担优惠比例值不能为空',
                                                        'max_value': u'business_ratio取值范围为1~100'})
